...
from bobverse.api.schemas.requests.comment import CreateCommentRequest
from bobverse.api.schemas.responses.comment import CommentResponse, CommentsListResponse
...
@router.get("/{slug}/comments/search")
async def search_comments(
    ...
    """
    Search comments with XSS vulnerability.
    
    WARNING: This endpoint is intentionally vulnerable to XSS for demo purposes.
    ...
    # The query parameter is directly included in the HTML response
    html_response = f"""
    <html>
        <head><title>Comment Search Results</title></head>
        <body>
            <h1>Search Results for: {query}</h1>
            <div>
                <p>Found {len(comment_list_dto.comments)} comments in article '{slug}'</p>
            </div>
        </body>
    </html>
    """
    
    # This is vulnerable because it returns HTML with unescaped user input
