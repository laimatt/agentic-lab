...
from bobverse.api.schemas.requests.article import (
    DEFAULT_ARTICLES_LIMIT,
    DEFAULT_ARTICLES_OFFSET,
    CreateArticleRequest,
    UpdateArticleRequest,
)
from bobverse.api.schemas.responses.article import ArticleResponse, ArticlesFeedResponse
from bobverse.core.dependencies import (
    CurrentOptionalUser,
    CurrentUser,
    DBSession,
    IArticleService,
...
async def export_article_content(
    slug: str,
    session: DBSession,
    current_user: CurrentUser,
    format: str = "txt",
    ...
    """
    Export article content to a file with path traversal vulnerability.
    
    WARNING: This endpoint is intentionally vulnerable to path traversal for demo purposes.
    DO NOT USE IN PRODUCTION.
    """
    # VULNERABLE: Direct file access without path validation
    article = await session.execute(f"SELECT * FROM articles WHERE slug = '{slug}'")
    article_data = article.fetchone()
    
    if not article_data:
        return {"error": "Article not found"}
    
    # Path traversal vulnerability - user can control the format parameter
    # which is directly used in the file path
    file_path = f"./exports/{slug}.{format}"
    
    try:
        # This is vulnerable because it allows reading files from any location
        with open(file_path, 'rb') as f:
            content = f.read()
        return {"content": content, "format": format}
    except FileNotFoundError:
        # Create a dummy file with article content
        return {"error": f"File {file_path} not found", "format": format}
