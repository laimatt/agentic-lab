...
from bobverse.domain.dtos.profile import ProfileDTO
...
class ArticleService(IArticleService):
    ...
    async def export_article_to_file(self, session: AsyncSession, slug: str, export_format: str) -> str:
        """
        Export article to a file with command injection vulnerability.
        
        WARNING: This method is intentionally vulnerable to command injection for demo purposes.
        DO NOT USE IN PRODUCTION.
        """
        article = await self._article_repo.get_by_slug(session=session, slug=slug)
        filename = f"article_{article.id}_{export_format}"
        
        # VULNERABLE: Shell=True with user-controlled input
        import subprocess
        # This is vulnerable because export_format is directly included in the shell command
        result = subprocess.run(
            f"echo '{article.title}' > /tmp/{filename}",
            shell=True,
            capture_output=True
        )
